...
use crate::{
    merge_state::{
        BoolOpMergeState, DetachConverter, DummyConverter, InPlaceVecMergeStateRef, MergeStateMut,
        NoConverter, VecMergeState, TT, TTI,
    },
    node::FlexRef,
...
impl<S: BlobStore> TreeNode<S> {
    ...
    fn scan_prefix<'a>(&self, store: &'a S, prefix: &[u8]) -> Result<Iter<'a, S>, S::Error> {
    ...
    fn group_by<'a, F: Fn(&[u8], &TreeNode<S>) -> bool>(
        &self,
        store: &'a S,
        descend: F,
    ) -> GroupBy<'a, S, F> {
    ...
    fn iter<'a>(&self, store: &'a S) -> Iter<'a, S> {
        let tree = TreeNode {
            prefix: TreePrefix::empty(),
            value: TreeValue::none(),
            children: TreeChildren::from_vec(vec![self.clone()]),
    ...
    fn values<'a>(&self, store: &'a S) -> Values<'a, S> {
...
impl Tree {
    ...
    pub fn retain_prefix_with(&mut self, that: &Tree, f: impl Fn(TreeValue) -> bool + Copy) {
        unwrap_safe(
            self.try_retain_prefix_with::<NoStore, NoError, _, _>(that, NoConverter, |b| Ok(f(b))),
        )
    }

    pub fn remove_prefix_with(&mut self, that: &Tree, f: impl Fn(TreeValue) -> bool + Copy) {
        unwrap_safe(
            self.try_remove_prefix_with::<NoStore, NoError, _, _>(that, NoConverter, |b| Ok(f(b))),
        )
    }
}
...
impl<S: BlobStore + Clone> Tree<S> {
        ...
        F: Fn(TreeValue<S>, TreeValue<S2>) -> Result<TreeValue, E> + Copy,
    {
        let node = outer_combine(
            TTI::<S, S2, E, DummyConverter>::default(),
            &self.node,
            &self.store,
            &that.node,
            &that.store,
...
fn inner_combine_pred<T: TT>(
    ...
    if n == ap.len() && n == bp.len() {
        if !a.value.is_none() && !b.value.is_none() {
            if f(a.value.clone(), b.value.clone())? {
                return Ok(true);
            }
        }
...
impl<'a, S: BlobStore> Iter<'a, S> {
    ...
    fn new(tree: TreeNode<S>, store: &'a S, prefix: IterKey) -> Self {
        Self {
            stack: vec![(tree, 0)],
            path: prefix,
            store,
...
pub struct Values<'a, S: BlobStore> {
    stack: Vec<(OwnedSlice<TreeNode<S>>, usize)>,
    store: &'a S,
}

impl<'a, S: BlobStore> Values<'a, S> {
    fn new(tree: TreeNode<S>, store: &'a S) -> Self {
        Self {
...
pub struct GroupBy<'a, S: BlobStore, F> {
    path: IterKey,
    stack: Vec<(TreeNode<S>, usize)>,
    store: &'a S,
    descend: F,
...
impl<'a, S: BlobStore, F: Fn(&[u8], &TreeNode<S>) -> bool> GroupBy<'a, S, F> {
    fn new(tree: TreeNode<S>, store: &'a S, prefix: IterKey, descend: F) -> Self {
        Self {
            stack: vec![(tree, 0)],
            path: prefix,
            store,
...
impl<'a, S: BlobStore, F: Fn(&[u8], &TreeNode<S>) -> bool> Iterator for GroupBy<'a, S, F> {
